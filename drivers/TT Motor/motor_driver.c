...
static ssize_t dev_write(struct file *filep, const char __user *buffer, size_t len, loff_t *offset) {
    ...
    // 根據字母直接調整 duty_ns，不使用容易出事的 pr_info
    if (strcmp(kbuf, "A") == 0 || strcmp(kbuf, "a") == 0) {
        duty_ns = 20000000; // 100%
    } 
    else if (strcmp(kbuf, "B") == 0 || strcmp(kbuf, "b") == 0) {
        duty_ns = 14000000; // 70%
    } 
    else if (strcmp(kbuf, "C") == 0 || strcmp(kbuf, "c") == 0) {
        duty_ns = 8000000;  // 40%
    } 
    else if (strcmp(kbuf, "D") == 0 || strcmp(kbuf, "d") == 0) {
        duty_ns = 0;        // 0%
    } 
    else {
        u64 custom_val = 0;
        if (kstrtoull(kbuf, 10, &custom_val) == 0) {
            duty_ns = custom_val;
        } else {
            return -EINVAL;
        }
